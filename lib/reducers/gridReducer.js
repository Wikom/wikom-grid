        ...
        case types.SETEDITROW:
            {
                ...
                _gridState2[action.name].edit.row = action.index || null;
        ...
        case types.DESTROY:
            {
                ...
                if (_gridState4[action.name]) {
                    delete _gridState4[action.name];
                }
        ...
        case _reactRouterRedux.LOCATION_CHANGE:
            {
                ...
                for (var _grid in _gridState5) {
                    if (!gridParams.hasOwnProperty(_grid)) {
                        console.log(defaultFilter);
                        _gridState5[_grid] = initialGridState();
                        _gridState5[_grid].filter = defaultFilter[_grid] || {};
                    }
                }
                ...
                for (var _grid2 in gridParams) {
                    ...
                    if (!_gridState5[_grid2]) {
                        _gridState5[_grid2] = initialGridState();
                    }

                    _gridState5[_grid2].data = [];
                    _gridState5[_grid2].selection = [];
                    _gridState5[_grid2].pagination.pageSize = gridConfig.pageSize || null;
                    _gridState5[_grid2].pagination.currentPage = gridConfig.currentPage || 1;
                    _gridState5[_grid2].sort = gridConfig.sort || null;
                    _gridState5[_grid2].filter = gridConfig.filter || Object.assign({}, _gridState5[_grid2].filter);
                }
        ...
        case types.DATA_CHANGED:
            {
                ...
                _gridState6[action.name].data = action.data;
        ...
        case types.SELECTION_CHANGED:
            {
                ...
                var availableIds = _gridState7[action.name].data.map(function (row) {
                    return row.id;
                });
                ...
                if (action.checked) {
                    _gridState7[action.name].selection = _gridState7[action.name].selection.concat(value).filter(function (v, i, a) {
                        return a.indexOf(v) === i;
                    });
                } else {
                    _gridState7[action.name].selection = _gridState7[action.name].selection.filter(function (v, i, a) {
