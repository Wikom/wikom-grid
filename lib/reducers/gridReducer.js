...
var gridReducer = function gridReducer() {
    ...
    switch (action.type) {
        ...
        case types.EDIT_START:
            {
                var _gridState7 = _extends({}, state);

                _gridState7[action.name].edit.rowId = action.rowId;
                _gridState7[action.name].edit.colId = action.colId;
        ...
        case _reduxForm.actionTypes.CHANGE:
            {
                ...
                if (nameParts[0] === 'gridedit') {
                    ...
                    if (!_gridState8[name].edit.status[rowId]) {
                        _gridState8[name].edit.status[rowId] = {};
                    }

                    var status = _gridState8[name].edit.status[rowId][colId];
                    ...
                    switch (action.type) {
                        ...
                        case _reduxForm.actionTypes.SET_SUBMIT_SUCCEEDED:
                            ...
                            if (!_gridState8[name].edit.values[rowId]) {
                                _gridState8[name].edit.values[rowId] = {};
                            }
                            _gridState8[name].edit.values[rowId][colId] = _gridState8[name].edit.tmp && _gridState8[name].edit.tmp[rowId] && _gridState8[name].edit.tmp[rowId][colId];
                            break;
                        case _reduxForm.actionTypes.SET_SUBMIT_FAILED:
                            status = submitStatus.SUBMIT_STATUS_FAILURE;
                            break;
                        case _reduxForm.actionTypes.CHANGE:
                            if (!_gridState8[name].edit.tmp[rowId]) {
                                _gridState8[name].edit.tmp[rowId] = {};
                            }
                            _gridState8[name].edit.tmp[rowId][colId] = action.payload;
                            break;
                    }

                    _gridState8[name].edit.status[rowId][colId] = status;
